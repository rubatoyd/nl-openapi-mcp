...
def as_phrase(kwd: str) -> str:
    ...
    🔴 **넓게 모으는 용도로 쓰면 안 된다.** 구문검색은 토큰 **인접**을 요구하는데, 한국어
       복합어는 실제 표제에서 조사·수식어로 갈라진다(`교육의 형평성`·`초중등교육의 형평성과`).
       실측(6개 검색어, 자료구분 도서): 재현율 손실 평균 **47%**, 최악 **84%**
       (`교육형평성` 31 → 5건). 버려진 249건 중 **76%가 구성어를 모두 포함한 관련 문헌**이었다.
       변형어를 3~5개로 늘려도 12건에 그쳐 기본 검색 31건에 못 미친다.

    용도: 전체 표제를 아는 **특정 자료 조회**, 동명이의 구분, 상한에 걸린 검색어의 total 을
    줄여 분할 수집을 돕는 보조 수단. 코퍼스 수집은 기본 검색 + `contains` 후처리로 한다.
    """
    k = (kwd or "").strip()
