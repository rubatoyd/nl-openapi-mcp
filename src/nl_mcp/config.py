...
# ── 500 상한을 쪼개는 데 쓸 수 있는 **서버측 축** (2026-08-12 실측 탐색) ──────────
# 응답 필드 이름을 그대로 파라미터로 넘겨보는 방식으로 찾았다. 대부분은 무시되고
# 아래 셋만 total 을 실제로 바꾼다. 무시되는 것: kdcCode1s·classNo·typeCode·typeName·
# menuName·mediaName·placeInfo·docYn·isbn·controlNo·regDate·lang·detailSearch·f1 …
# ⚠️ `kdcCode1s`(코드)는 무시되는데 `kdcName1s`(이름)는 동작한다. `licYn` 은 되는데
#    `docYn` 은 안 된다 — 이름만 보고 대칭을 가정하면 안 된다.

# 관리기관 — ✅ **완전분할**(교육복지/도서에서 974+872+10 = 1,856 = 전체와 정확히 일치)
MANAGE_NAMES = ["본관", "디지털도서관", "어린이청소년도서관", "국립세종도서관"]

# 이용조건 코드 — ⚠️ **손실 있는 분할.** 값이 **빈 레코드가 존재**하며 어떤 값으로도
# 선택할 수 없다(교육복지/도서 본관 974건 중 A~Z 전수 탐색으로 472건만 선택 가능,
# 나머지 502건은 licYn 이 비어 도달 불가). 그래도 조각이 작아 회복량은 가장 크다.
LIC_CODES = ["D", "F", "L", "N", "S", "U", "Y"]

# KDC 대분류명 — 동작하지만 한 값(사회과학)에 쏠려 분할 효과가 낮아 기본 축에서 제외.
KDC_NAMES = ["총류", "철학", "종교", "사회과학", "자연과학", "기술과학",
             "예술", "언어", "문학", "역사", "기타"]

# 분할 축 적용 순서 — 완전분할인 축을 먼저 쓰고, 그래도 상한에 걸린 조각만 다음 축으로 쪼갠다.
PARTITION_AXES = [
    ("category", CATEGORIES),
    ("manageName", MANAGE_NAMES),
    ("licYn", LIC_CODES),
]
