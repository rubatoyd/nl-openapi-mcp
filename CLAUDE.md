...
**같은 검색식 안에서는 페이지네이션으로 501번째에 도달할 수 없다.** 전수 수집은
검색어 세분화·`category` 분할 등 **검색 공간 분할**로만 가능하다.
`year_from`/`contains` 는 **로컬 후처리**라 상한을 풀어주지 않는다 → [가이드 §3](docs/NL_API_GUIDE.md).
