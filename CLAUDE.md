...
- 🔬 **kci 하네스 이식이 역방향 지적을 냈다 (2026-08-12)** — kci 에서 **인증키가 MCP 응답까지
  새는 것**을 잡았는데(오류 봉투가 요청을 에코 → 파서가 전 요소 텍스트를 이어붙임), 여기도
  `nl_status` 가 **응답 봉투를 그대로 도구 응답에 싣는다**. 이 API 가 `key` 를 에코하는지는
  **라이브로 확정하지 못했다**(확인 시도 중 www.nl.go.kr 이 다시 불통 — 세션 내내 간헐적).
  수집해 둔 실응답 1,124건의 봉투에는 없었으나 **오류 봉투는 미확인**이다.
  → `parser._envelope()` 에서 자격증명형 키(`key`·`apiKey`·`token`…)를 제거한다. 값이 없으면
    아무 일도 없고, 있으면 막힌다. 회귀 2건 추가(186 → **188**).
  ⚠️ 세 저장소 모두 `requests` 예외를 **타입만 남기고 버린다**는 것은 재확인했다(정상).
    실제로 이 세션에서 임시 진단 스크립트가 SSLError 를 그대로 찍어 **인증키를 노출**했는데,
    라이브러리 경로였다면 나지 않았을 일이다 — **일회성 스크립트에도 같은 규율을 적용할 것.**
