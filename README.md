...
| `cap_hit` | `total` > 500 — **API 가 더 안 줌** | `max_records` 로는 불가. 검색식을 쪼갤 것 |
| `meta.cap_hit_terms` | 상한에 걸린 검색어 목록 | 그 검색어만 세분화 |

> ⚠️ `year_from`/`contains` 같은 필터는 **이미 받은 레코드에 대한 후처리**라 상한을 풀어주지 않습니다.
> 자세한 우회 전략 → [docs/NL_API_GUIDE.md §3](docs/NL_API_GUIDE.md)
